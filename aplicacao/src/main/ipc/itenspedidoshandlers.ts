...
export function ItensPedidoController() {
  ...
  ipcMain.handle(
    ...
    ipcLogger('get-itens-mais-pedidos', pedido_itens.getAlimentosMaisPedidos)
  )
  ipcMain.handle(
    'get-item-id-pedidos',
    ipcLogger('get-item-id-pedidos', (__event, itemId) => pedido_itens.getItemIdPedidos(itemId))
  )
  ipcMain.handle(
    'get-itens-pedido-id',
    ipcLogger('get-itens-pedido-id', (__event, pedidoId) => pedido_itens.getItensPedidoId(pedidoId))
  )
  ipcMain.handle(
